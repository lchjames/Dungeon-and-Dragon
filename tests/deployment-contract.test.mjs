...
assert.match(
  wrangler,
  /^\s*"main"\s*:\s*"\.\/src\/runtime-object-gateway\.js"\s*,?\s*$/m,
...
assert.match(objectGateway, /ensureRuntimeObjectAuthority/);
assert.match(objectGateway, /processPendingObjectStoryEvents/);
assert.match(objectGateway, /processPendingRuntimeStoryLifecycleEvents/);
