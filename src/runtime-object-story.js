...
const MAX_OBJECT_OCCURRENCES_PER_DRAIN = 50;
...
async function applyEffect(env, context, effect, effectIndex) {
  ...
  if (effect.type === 'open_door' || effect.type === 'close_door') {
    ...
    return {
      type: effect.type,
      ...(await applyDoorEffect(env, context, edge, effect.type === 'open_door' ? 'open' : 'closed'))
    };
  }
...
async function writeDispatch(env, occurrenceId, eventId, result) {
  ...
  `).bind(
    `story_dispatch_${crypto.randomUUID()}`,
    occurrenceId,
    eventId,
    result.status,
...
async function claimNextOccurrence(env, sceneRunId) {
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    const row = await env.DB.prepare(`
      ...
      WHERE scene_run_id = ?
        AND trigger_type = 'interact_object'
    ...
    if (Number(result?.meta?.changes || 0) === 1) {
      return { ...row, lease_token: leaseToken, lease_at: now };
    }
  }
  return null;
}
...
async function completeOccurrence(env, occurrence) {
  ...
  if (Number(result?.meta?.changes || 0) !== 1) {
    throw Object.assign(new Error('Object Story lifecycle occurrence lease changed before completion.'), {
...
async function processOccurrence(env, occurrence) {
  if (occurrence.subject_type !== 'object_interaction') {
    throw Object.assign(new Error('interact_object occurrence has an invalid subject type.'), {
      code: 'STORY_LIFECYCLE_SUBJECT_INVALID'
    });
  }
  const interaction = await env.DB.prepare(`
    SELECT roi.*, rmo.state_key AS current_state_key, rmo.interaction_count AS current_interaction_count,
           rmo.name_snapshot, rmo.object_type, rmo.x, rmo.y,
           rmi.scene_id, rmi.status AS map_status, sr.status AS scene_run_status
    FROM runtime_object_interaction_log roi
    JOIN runtime_map_objects rmo ON rmo.id = roi.runtime_object_id
    JOIN runtime_map_instances rmi ON rmi.id = roi.map_instance_id
    JOIN scene_runs sr ON sr.id = roi.scene_run_id
    WHERE roi.id = ? AND roi.scene_run_id = ?
    LIMIT 1
  `).bind(occurrence.subject_id, occurrence.scene_run_id).first();
  if (!interaction
      || interaction.map_status !== 'active'
      || interaction.scene_run_status !== 'active'
      || interaction.actor_user_id !== occurrence.actor_user_id
      || Number(interaction.created_at) !== Number(occurrence.source_at)) {
    throw Object.assign(new Error('interact_object occurrence does not point to a valid committed Runtime Object interaction.'), {
      code: 'STORY_LIFECYCLE_OBJECT_INTERACTION_INVALID'
    });
  }
  ...
  const [eventRows, targets, flags, counts, encounters, alreadyDispatched] = await Promise.all([
    env.DB.prepare(`
      ...
      WHERE scene_id = ? AND status = 'active' AND trigger_type = 'interact_object' AND created_at <= ?
      ORDER BY created_at, id
    `).bind(interaction.scene_id, occurrence.source_at).all(),
    loadTargets(env, interaction.map_instance_id),
    loadFlags(env, interaction.scene_run_id),
    appliedCounts(env, interaction.scene_run_id),
    loadRuntimeEncounterMap(env, interaction.scene_run_id, interaction.scene_id),
  ...
  const shared = {
    ...
    sceneRunStatus: interaction.scene_run_status,
    sceneId: interaction.scene_id,
    mapInstanceId: interaction.map_instance_id,
    targets,
    flags,
    ...
    lifecycleSourceObjectId: interaction.source_object_id,
    lifecycleRuntimeObjectId: interaction.runtime_object_id,
    lifecycleCharacterId: interaction.character_id,
    lifecycleInteractionMode: interaction.interaction_mode,
  ...
  const metadata = {
    ...
    objectInteractionId: interaction.id,
    sourceObjectId: interaction.source_object_id,
    runtimeObjectId: interaction.runtime_object_id,
    objectName: interaction.name_snapshot,
    objectType: interaction.object_type,
    objectStateBefore: interaction.from_state_key,
    objectStateAfter: interaction.to_state_key,
    characterId: interaction.character_id,
    interactionMode: interaction.interaction_mode,
  ...
  for (const row of eventRows.results || []) {
    ...
    let trigger;
    try {
      trigger = normalizeStoryTrigger('interact_object', event.trigger);
    } catch (error) {
...
export async function processPendingObjectStoryEvents(env, { sceneRunId } = {}) {
  if (!sceneRunId) return [];
  await ensureSchema(env);
  const allResults = [];
  for (let processed = 0; processed < MAX_OBJECT_OCCURRENCES_PER_DRAIN; processed += 1) {
    const occurrence = await claimNextOccurrence(env, sceneRunId);
    if (!occurrence) return allResults;
    try {
      allResults.push(...(await processOccurrence(env, occurrence)));
  ...
  const pending = await env.DB.prepare(`
    ...
    WHERE scene_run_id = ? AND trigger_type = 'interact_object' AND completed_at IS NULL
    LIMIT 1
  `).bind(sceneRunId).first();
  if (pending) {
    throw Object.assign(new Error('Runtime Object Story lifecycle cascade exceeded the per-request safety limit.'), {
