...
export function createRuntimeMapObjectWorker(baseWorker) {
  ...
  async function definitionObjects(env, mapId) {
    const rows = await env.DB.prepare('SELECT * FROM map_objects WHERE map_template_id = ? ORDER BY created_at, id').bind(mapId).all();
    return (rows.results || []).map(definitionObjectPayload);
  }
  ...
  async function loadDefinitionObjects(request, env, mapId) {
    ...
    return json({
      ...
      objects: await definitionObjects(env, mapId)
    });
  }
    ...
    const results = await env.DB.batch([
      ...
      env.DB.prepare(`INSERT INTO map_objects
        ...
        SELECT json_extract(value, '$.id'), ?, json_extract(value, '$.name'),
               json_extract(value, '$.objectType'), CAST(json_extract(value, '$.x') AS INTEGER),
               CAST(json_extract(value, '$.y') AS INTEGER), CAST(json_extract(value, '$.interactionRange') AS INTEGER),
               CAST(json_extract(value, '$.playerVisibleDefault') AS INTEGER),
               CAST(json_extract(value, '$.enabledDefault') AS INTEGER),
               json_extract(value, '$.initialStateJson'), json_extract(value, '$.gmNotes'), ?, ?
        FROM json_each(?)
        WHERE EXISTS (SELECT 1 FROM map_templates WHERE id = ? AND version = ?)`)
        .bind(mapId, now, now, objectsJson, mapId, expectedVersion),
      env.DB.prepare(`UPDATE map_templates SET version = version + 1, updated_at = ?
        WHERE id = ? AND version = ?`).bind(now, mapId, expectedVersion)
    ]);
  ...
  async function runtimeObjects(env, mapInstanceId) {
    const rows = await env.DB.prepare(`SELECT * FROM runtime_map_objects
      WHERE map_instance_id = ? ORDER BY y, x, created_at, id`).bind(mapInstanceId).all();
  ...
  async function loadRuntimeObjects(request, env, mapInstanceId) {
    await requireGM(request, env);
    await ensureObjectSchema(request, env);
    const map = await env.DB.prepare(`SELECT id, map_name_snapshot, scene_id, scene_run_id, status, width, height,
      source_map_version FROM runtime_map_instances WHERE id = ? LIMIT 1`).bind(mapInstanceId).first();
    ...
    async fetch(request, env) {
      ...
      try {
        ...
        if (startsRuntime || (runtimeDetailMatch && request.method === 'GET')) {
          await ensureObjectSchema(request, env);
          return await augmentRuntimeResponse(await baseWorker.fetch(request, env), env);
        }
        return baseWorker.fetch(request, env);
      } catch (error) {
        console.error('Runtime Map Object layer error', {
          path: pathname,
