...
const TRIGGER_TYPE = 'encounter_activated';
...
async function writeDispatch(env, occurrenceId, eventId, result) {
  const now = Date.now();
  const failure = result.status === 'failed' ? { code: result.code || null, message: result.message || null } : { code: result.code || null, message: null };
  await env.DB.prepare(`
...
async function claimNextOccurrence(env, sceneRunId) {
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    const row = await env.DB.prepare(`
      SELECT * FROM runtime_story_lifecycle_occurrences
      WHERE scene_run_id = ? AND trigger_type = ? AND completed_at IS NULL
        AND (lease_token IS NULL OR lease_at IS NULL OR lease_at < ?)
      ORDER BY source_at, created_at, id
      LIMIT 1
    `).bind(sceneRunId, TRIGGER_TYPE, staleBefore).first();
    if (!row) return null;
...
async function processOccurrence(env, occurrence) {
  ...
  if (!runtime) {
    throw Object.assign(new Error('Active Runtime Map is unavailable for encounter_activated Story dispatch.'), {
  ...
  const [eventRows, targets, flags, counts, encounters, alreadyDispatched] = await Promise.all([
    env.DB.prepare(`
      ...
      ORDER BY created_at, id
    `).bind(sceneRun.scene_id, TRIGGER_TYPE, occurrence.source_at).all(),
  ...
  const activatedEncounter = encounters.get(occurrence.subject_id);
  if (!activatedEncounter || activatedEncounter.status !== 'active') {
  ...
  const shared = {
    ...
    encounters,
    activatedEncounterId: occurrence.subject_id
  };
  const results = [];
  for (const row of eventRows.results || []) {
    ...
    let trigger;
    try {
      trigger = normalizeStoryTrigger(TRIGGER_TYPE, event.trigger);
    } catch (error) {
      if (!alreadyDispatched.has(event.id)) {
        const failed = { eventId: event.id, name: event.name, status: 'failed', ...cleanError(Object.assign(error, { code: 'STORY_TRIGGER_INVALID' })) };
        await writeDispatch(env, occurrence.id, event.id, failed);
        results.push({ ...failed, encounterId: occurrence.subject_id, occurrenceId: occurrence.id });
      }
      continue;
    }
    if (trigger.encounterId !== occurrence.subject_id || alreadyDispatched.has(event.id)) continue;
    ...
    results.push({ ...result, encounterId: occurrence.subject_id, occurrenceId: occurrence.id });
  }
  return results;
}
...
export async function processPendingEncounterActivatedStoryEvents(env, { sceneRunId } = {}) {
  ...
  const pending = await env.DB.prepare(`
    ...
    WHERE scene_run_id = ? AND trigger_type = ? AND completed_at IS NULL
    LIMIT 1
  `).bind(sceneRunId, TRIGGER_TYPE).first();
  if (pending) {
    throw Object.assign(new Error('encounter_activated Story cascade exceeded the per-request safety limit.'), {
