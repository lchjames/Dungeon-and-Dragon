...
> 本文件覆蓋較早 `Rank 1 = 2 MP ... Rank 9 = 32 MP` 的固定成本表，以及需要為每種普通拳腳／揮劍建立額外 Ability 層的設計。
...
# 5. MP 成本改用 Max MP 比例，而不是固定 2→32
...
原因：固定差距過平，而且當角色 Max MP 改變時，成本尺度會失去意義。

Alpha Default Reference 改為：

| Ability Rank | Default MP Cost Ratio |
|---:|---:|
| 1 | 3% Max MP |
| 2 | 5% Max MP |
| 3 | 8% Max MP |
...
```text
Default MP Cost
= max(1, ceil(Max MP × Rank Cost Ratio))
```
...
```text
Rank 9 MP Cost 是 Rank 1 的 X 倍
→ Rank 9 Damage 必須剛好是 Rank 1 的 X 倍
```
...
因此高 Rank Ability 的總 Power 差距可以遠高於單純 MP Cost 的倍數。
...
```text
建立／修改 Ability
→ AI 參考 Rank Default MP Ratio + Ability Power Package
→ 建議實際 MP Cost 並簡短解釋
→ Player 確認
→ GM 批准／修改
...
7. 舊固定 `2→32 MP` Rank 表取消。
8. 新 Default Cost 使用 Max MP 比例：`3%, 5%, 8%, 13%, 20%, 30%, 45%, 65%, 100%`。
9. MP Cost 不需要與 Damage 倍率一比一；Rank Power 還包括範圍、控制、治療、多段、持續等總效果。
10. AI 只在 Ability 建立／修改時提出 MP Cost；GM 批准後固定保存於 D1，實際每次施放不需要 AI 即時計算。
11. 不為這套簡化額外新增 Stamina、氣力或另一條普通攻擊技能樹。
12. 所有正式 Ability、MP Cost、Profile 與 GM 裁決保存於 D1。
