...
# 12. Next Unresolved Boss Decision

The next Boss-specific architecture decision is whether a Boss Design Profile is primarily:

```text
reusable as a Template for multiple Boss instances
or
one designed Boss record tied to a specific encounter / identity
or
supports both reusable Profile and spawned Instance layers
```
